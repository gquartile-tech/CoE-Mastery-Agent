...
def classify_breadth(n: int) -> str:
    if n <= 5:
        return 'narrow'
    if n <= 20:
        return 'mid-sized'
...
def _evaluate_all_inner(ctx: DatabricksContext) -> Dict[str, ControlResult]:
    ...
    p_ok = bool(clean_text(ctx.p7)); q_ok = bool(clean_text(ctx.q7))
    if p_ok and q_ok:
        r['C007'] = ControlResult('OK', '', WHY['C007'], SOURCES['C007'])
    elif p_ok or q_ok:
        r['C007'] = ControlResult('PARTIAL', 'Only one of the required core ownership fields is documented in 54_Project_Dataset_on_SF!P7/Q7.', WHY['C007'], SOURCES['C007'])
    else:
        r['C007'] = ControlResult('FLAG', 'Required core ownership fields are not documented in 54_Project_Dataset_on_SF!P7/Q7.', WHY['C007'], SOURCES['C007'])
    ...
    if checks == [True, True]:
        ...
        r['C008'] = ControlResult('PARTIAL', f'Constraint alignment is mixed. ACoS constraint {pct_str(acos_c)} vs project target {pct_str(proj_acos)}; TACoS constraint {pct_str(tacos_c)} vs project target {pct_str(proj_tacos)}.', WHY['C008'], SOURCES['C008'])
    elif any(v is False for v in checks):
        r['C008'] = ControlResult('FLAG', f'Project KPI targets are looser than the current account constraints. ACoS constraint {pct_str(acos_c)} vs project target {pct_str(proj_acos)}; TACoS constraint {pct_str(tacos_c)} vs project target {pct_str(proj_tacos)}.', WHY['C008'], SOURCES['C008'])
    else:
        r['C008'] = ControlResult('PARTIAL', f'Constraint alignment could not be fully validated. ACoS constraint {pct_str(acos_c)} vs project target {pct_str(proj_acos)}; TACoS constraint {pct_str(tacos_c)} vs project target {pct_str(proj_tacos)}.', WHY['C008'], SOURCES['C008'])

    if ctx.top1 is None:
        r['C009'] = ControlResult('FLAG', 'Sales concentration could not be evaluated because parent-ASIN sales data was unavailable.', WHY['C009'], SOURCES['C009'])
    else:
        ...
        if narr_class == actual_class:
            ...
            r['C009'] = ControlResult('FLAG', f'Sales concentration is not documented in AU7. Actual concentration is {actual_class} (top1 {pct_str(ctx.top1)}, top3 {pct_str(ctx.top3)}, top5 {pct_str(ctx.top5)}).', WHY['C009'], SOURCES['C009'])
        else:
            r['C009'] = ControlResult('FLAG', f'Sales concentration narrative does not match actual concentration. AU7 says {ctx.au or "not documented"}; actual concentration is {actual_class} (top1 {pct_str(ctx.top1)}, top3 {pct_str(ctx.top3)}, top5 {pct_str(ctx.top5)}).', WHY['C009'], SOURCES['C009'])

    actual_count = ctx.parent_count; n = to_float(ctx.ap)
    if actual_count is None:
        r['C010'] = ControlResult('FLAG', 'Parent ASIN breadth could not be evaluated because 07_KPIs_by_Parent_ASIN_by_Month was unavailable.', WHY['C010'], SOURCES['C010'])
    elif n is None:
        r['C010'] = ControlResult('FLAG', f'AP7 does not contain a usable Parent ASIN count, while actual distinct ParentASIN count is {actual_count}.', WHY['C010'], SOURCES['C010'])
    else:
        n_int = int(round(n))
        if abs(n_int - actual_count) <= max(2, int(round(actual_count * 0.2))):
            r['C010'] = ControlResult('OK', '', WHY['C010'], SOURCES['C010'])
        elif classify_breadth(n_int) == classify_breadth(actual_count) or abs(n_int - actual_count) <= 5:
            r['C010'] = ControlResult('PARTIAL', f'AP7 reports {n_int} Parent ASINs, while actual distinct ParentASIN count is {actual_count}.', WHY['C010'], SOURCES['C010'])
        else:
            r['C010'] = ControlResult('FLAG', f'Parent ASIN breadth narrative does not match actual breadth. AP7 reports {n_int}; actual distinct ParentASIN count is {actual_count}.', WHY['C010'], SOURCES['C010'])

    if ctx.gap is None:
        if ctx.last_call is not None:
            r['C011'] = ControlResult('PARTIAL', f'Only one valid Gong meeting date was found ({ctx.last_call.date()}); cadence could not be assessed from two meetings.', WHY['C011'], SOURCES['C011'])
        else:
            r['C011'] = ControlResult('FLAG', 'No valid Gong meeting dates were found in column P.', WHY['C011'], SOURCES['C011'])
    else:
        if ctx.gap <= 30:
            r['C011'] = ControlResult('OK', '', WHY['C011'], SOURCES['C011'])
        elif ctx.gap <= 60:
            r['C011'] = ControlResult('PARTIAL', f'Latest Gong meeting spacing is {ctx.gap} days ({ctx.prev_call.date()} to {ctx.last_call.date()}).', WHY['C011'], SOURCES['C011'])
        else:
    ...
    if active_count == 0:
        ...
        if documented_count >= active_count:
            ...
            r['C012'] = ControlResult('PARTIAL', f'Active personalization was detected across {active_count} area(s) ({labels}), with partial documentation in CS Notes.', WHY['C012'], SOURCES['C012'])
        else:
            r['C012'] = ControlResult('FLAG', f'Active personalization was detected across {active_count} area(s) ({labels}), but CS Notes do not sufficiently document the setup.', WHY['C012'], SOURCES['C012'])
    ...
    if not checks:
        ...
        r['C013'] = ControlResult('FLAG', 'Target vs actual performance shows significant deviation across key KPIs.', WHY['C013'], SOURCES['C013'])
    else:
        r['C013'] = ControlResult('PARTIAL', 'Target vs actual performance shows moderate deviation across key KPIs.', WHY['C013'], SOURCES['C013'])
    ...
    if has_best and has_category:
        ...
        r['C014'] = ControlResult('PARTIAL', 'Product tagging is present, but bestseller or category segmentation is only partially identifiable.', WHY['C014'], SOURCES['C014'])
    else:
        r['C014'] = ControlResult('FLAG', 'No clear product tagging or segmentation logic was identified in the campaign tag fields.', WHY['C014'], SOURCES['C014'])
...
def compute_score(results: Dict[str, ControlResult]):
    ...
    for cid, res in results.items():
        ...
        total_penalty += pen
        # C015 and C016 are manual controls — exclude from findings list entirely
